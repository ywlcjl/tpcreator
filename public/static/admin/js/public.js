...
function filterForm() {
    $('#filterFormTitle').click(function() {
...
function deleteAttach(obj) {
    ...
    if (result) {
        ...
        $.post(MODULE_URL + "attach/ajaxDelete", {
            'id': id
...
function articleUpload() {
    $('#buttonUpload').click(function () {
        ...
        formData.append('image', $("#inputFile").get(0).files[0]);

        $.ajax({
            url: MODULE_URL+'attach/ajaxUpload',
            data: formData,
            contentType: false,
            processData: false,
            type: 'POST',
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    str += '<a href="/' + data.picUrl + '" target="_blank" class="bd_attach_img"><img src="/' + data.picUrlThumb + '" class="img-thumbnail" alt="' + data.orig_name + '"></a> ';
